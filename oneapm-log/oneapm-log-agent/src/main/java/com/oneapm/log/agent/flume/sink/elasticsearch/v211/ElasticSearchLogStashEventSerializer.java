...
/**
 * Serialize flume events into the same format LogStash uses</p> This can be used to send events to
 * ElasticSearch and use clients such as Kabana which expect Logstash formated indexes
 *
 * <pre>
 * {
 ...
 * </pre>
 *
 * If the following headers are present, they will map to the above logstash output as long as the
 * logstash fields are not already present.</p>
 *
 * <pre>
...
public class ElasticSearchLogStashEventSerializer implements ElasticSearchEventSerializer {
    ...
    private void appendBody(XContentBuilder builder,
                            Event event) throws IOException, UnsupportedEncodingException {
        byte[] body = event.getBody();
        ContentBuilderUtil.appendField(builder,
                                       "@message",
                                       body);
    }
    
    private void appendHeaders(XContentBuilder builder,
                               Event event) throws IOException {
        // modify Maps to guava.Maps, for bug in es-2.1.1
