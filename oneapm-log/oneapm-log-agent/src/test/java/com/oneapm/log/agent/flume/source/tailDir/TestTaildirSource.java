...
import static com.oneapm.log.agent.flume.source.tailDir.TaildirSourceConfigurationConstants.*;
...
public class TestTaildirSource {
  ...
  public void testFileConsumeOrder() throws IOException {
    ...
    try {
      Thread.sleep(500); // wait before creating a new file
    ...
    try {
      Thread.sleep(500); // wait before creating next file
    } catch (InterruptedException e) {
    }
    ...
    Context context = new Context();
    context.put(POSITION_FILE, tmpDir.getAbsolutePath() + "/taildir_position_test.json");
