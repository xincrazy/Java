...
/**
 * Default index name builder. It prepares name of index using configured prefix and current
 * timestamp. Default format of name is prefix-yyyy-MM-dd".
 */
public class TimeBasedIndexNameBuilder implements IndexNameBuilder {
    ...
    public String getIndexName(Event event) {
        // box the Event to TimestampedEvent and parse the timestamp/indexname/indextype
        TimestampedEvent timestampedEvent = new TimestampedEvent(event);
        long timestamp = timestampedEvent.getTimestamp();
        String indexname = timestampedEvent.getIndexName();
        // get the real indexname prefix
        String realIndexPrefix = (StringUtils.isEmpty(timestampedEvent.getIndexName()) ||
                                  StringUtils.isEmpty(timestampedEvent.getIndexType()) || timestampedEvent.getDocument() == null)
                                                                                                                                 ? BucketPath.escapeString(indexPrefix,
                                                                                                                                                           event.getHeaders())
                                                                                                                                 : indexname;
        return new StringBuilder(realIndexPrefix).append('-')
                                                 .append(fastDateFormat.format(timestamp))
                                                 .toString();
    }
    
    @Override
    public String getIndexPrefix(Event event) {
        // box the Event to TimestampedEvent and parse the indexname/indextype
        TimestampedEvent timestampedEvent = new TimestampedEvent(event);
        String indexname = timestampedEvent.getIndexName();
        // get the real indexname prefix
        return (StringUtils.isEmpty(timestampedEvent.getIndexName()) ||
                StringUtils.isEmpty(timestampedEvent.getIndexType()) || timestampedEvent.getDocument() == null)
                                                                                                               ? BucketPath.escapeString(indexPrefix,
                                                                                                                                         event.getHeaders())
                                                                                                               : indexname;
    ...
    public void configure(Context context) {
        ...
        // indexName config in .conf file in here for set indexPrefix, to build full indexname
    ...
    /**
     * @see com.oneapm.log.agent.flume.sink.elasticsearch.v211.IndexNameBuilder#getIndexType(org.apache.flume.Event)
     */
    @Override
    public String getIndexType(Event event) {
        ...
        return (StringUtils.isEmpty(timestampedEvent.getIndexName()) ||
                StringUtils.isEmpty(timestampedEvent.getIndexType()) || timestampedEvent.getDocument() == null)
                                                                                                               ? null
                                                                                                               : timestampedEvent.getIndexType();
    }
    
    /**
     * @see com.oneapm.log.agent.flume.sink.elasticsearch.v211.IndexNameBuilder#getDocument(org.apache.flume.Event)
     */
    @Override
