...
/**
 * Rest ElasticSearch client which is responsible for sending bulks of events to ElasticSearch using
 * ElasticSearch HTTP API. This is configurable, so any config params required should be taken
 * through this.
 */
public class ElasticSearchRestClient implements ElasticSearchClient {
    ...
    public void addEvent(Event event,
        ...
        indexParameters.put(INDEX_PARAM,
                            indexNameBuilder.getIndexName(event));
        indexParameters.put(TYPE_PARAM,
                            indexType);
        if (ttlMs > 0) {
            indexParameters.put(TTL_PARAM,
                                Long.toString(ttlMs));
        }
        parameters.put(INDEX_OPERATION_NAME,
                       indexParameters);
        ...
        synchronized (bulkBuilder) {
            ...
            bulkBuilder.append("\n");
            bulkBuilder.append(content.toBytesArray()
                                      .toUtf8());
            bulkBuilder.append("\n");
        }
    }
    ...
    public void execute() throws Exception {
        ...
        while (statusCode != HttpStatus.SC_OK &&
            ...
            statusCode = response.getStatusLine()
                                 .getStatusCode();
            logger.info("Status code from elasticsearch: " +
                        statusCode);
        ...
        if (statusCode != HttpStatus.SC_OK) {
            if (response.getEntity() != null) {
                throw new EventDeliveryException(EntityUtils.toString(response.getEntity(),
                                                                      "UTF-8"));
            } else {
                throw new EventDeliveryException("Elasticsearch status code was: " +
                                                 statusCode);
            }
        }
    }
