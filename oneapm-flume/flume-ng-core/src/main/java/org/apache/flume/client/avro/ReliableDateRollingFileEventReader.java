...
public class ReliableDateRollingFileEventReader implements ReliableEventReader {

    private static final Logger logger = LoggerFactory
            .getLogger(ReliableDateRollingFileEventReader.class);
    ...
    /**
     * Create a ReliableDateRollingFileEventReader to watch the given directory.
     */
    private ReliableDateRollingFileEventReader(File spoolDirectory,
                                            ...
                                            ConsumeOrder consumeOrder, String currentFileNameFormat) throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, metaDir={}, " +
                            "deserializer={}",
                    new Object[] { ReliableDateRollingFileEventReader.class.getSimpleName(),
        ...
        this.currentFileNameFormat = currentFileNameFormat;
     ...
     * Loop until events is not empty or there is no next file in case of 0 byte files */
        while (events.isEmpty()) {
            //  Only CurrentFile is no longer the target, then try to retire the current file and set it to the next new log file.
            if (!isCurrentFileTargetLogFile(currentFile)) {
                logger.info("File:{} is no longer a TARGET Log File, which will be retired.",
            ...
            FileFilter filter = new FileFilter() {
                public boolean accept(File candidate) {
                    ...
                    if ((candidate.isDirectory()) ||
                            ...
                            !(fileName.contains(today) || fileName.contains("current"))) {
                        return false;
                    }
                    return true;
                }
    ...
    private boolean isCurrentFileTargetLogFile(Optional<FileInfo> currentFile) {
        String inputFilename = currentFile.get().getFile().getName();
        SimpleDateFormat dateFormat = new SimpleDateFormat(this.currentFileNameFormat);
        String today = dateFormat.format(new Date());

        if (inputFilename.toLowerCase().contains(today.toLowerCase())
                || inputFilename.toLowerCase().contains(this.currentFileNameFormat)) {
            return true;
        }
    ...
    /**
     * Special builder class for ReliableDateRollingFileEventReader
     */
    public static class Builder {
        ...
        public Builder currentFileNameFormat(String currentFileNameFormat) {
            this.currentFileNameFormat = currentFileNameFormat;
            return this;
        }

        public ReliableDateRollingFileEventReader build() throws IOException {
            return new ReliableDateRollingFileEventReader(spoolDirectory, completedSuffix,
                    ...
                    consumeOrder, currentFileNameFormat);
        }
    }
